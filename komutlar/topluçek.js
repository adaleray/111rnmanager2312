module.exports.operate = async ({client, msg, args, author, uye, cfg}) => {
  ...
  if (!args[1]) {
    ...
    var kanal2 = msg.guild.channels.cache.get(author.voice.channel.id);
    var uyeler = kanal.members.cache.array();
    if (!author.voice.channel) return msg.channel.send("**Bu komutu kullanmak için bir kanalda olmalısın.**").then(msj => msj.delete({ timeout: 5000 }));
    ...
    if (kanal.id === kanal2.id) return msg.channel.send("**Zaten aynı kanaldasınız.**").then(msj => msj.delete({ timeout: 5000 }));
