module.exports.operate = async ({client, msg, args, db}) => {
  ...
  if (!ayar) {
    ...
    if (sicil.length > 0) {
      ...
      await msg.channel.send({embed:{timestamp:new Date(),color:Math.floor(Math.random()*(0xFFFFFF+1)),description:`**${kisi} adlı üyenin sicili temizlendi.**`}}).then(msj => msj.delete({timeout: 5000}));
    } else {
      await msg.channel.send({embed:{timestamp:new Date(),color:Math.floor(Math.random()*(0xFFFFFF+1)),description:`**${kisi} adlı üyenin sicili zaten temiz.**`}}).then(msj => msj.delete({timeout: 5000}));
    };
  };
};
