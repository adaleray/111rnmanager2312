...
module.exports.operate = async ({client, msg, args, db}) => {
  ...
  if (!ayar) {
    ...
    let liste = sicil.length > 0 ? sicil.map((value, index) => `\`${index + 1}.\` **[${value.tip}]** ${client.toDate(value.zaman)} tarihinde **${value.sebep}** sebebi ile ${msg.guild.members.cache.has(value.yetkili) ? msg.guild.members.cache.get(value.yetkili) : value.yetkili} tarafından atıldı.`).join("\n") : "Temiz!";
