module.exports.operate = async ({client, msg, args, cfg, author}) => {
  ...
  msg.channel.send({
   embed: { description:`\`Genel:\` ${
    client.emojili(`${ortalama(genel)}`)
    }\n\`Erkek:\` ${
    client.emojili(`${ortalama(erkek)}`)
    }\n\`Kız:\` ${client.emojili(`${ortalama(kiz)}`)}\n\`Ekip:\` ${client.emojili(`${ortalama(tagli)}`)}\n\`Ses:\` ${client.emojili(`${ortalama(ses)}`)}
    `, color: client.favoriRenkler[Math.floor(Math.random() * client.favoriRenkler.length)], timestamp: new Date(), author: { name: msg.guild.name, icon}
                   } });
};
