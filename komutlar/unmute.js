module.exports.operate = async ({msg, uye, author, cfg}) => {
  ...
  if (uye.roles.cache.get(cfg.roles.ceza.muted)) {
    ...
    await muteLog.send({ embed: { description: `${uye} adlı üyenin mutesi ${author} tarafından kaldırıldı.` , color: "BLACK", timestamp: new Date() }});
  } else {
